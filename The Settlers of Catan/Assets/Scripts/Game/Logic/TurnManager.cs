...
public class TurnManagerOld : PunTurnManager
{
    ...
    private Queue<PhotonView> playerQueue = new Queue<PhotonView>();
    private Stack<PhotonView> playerStack = new Stack<PhotonView>();
    ...
    void Update()
    {
        ...
        // This selects the master client cause the turn manager is owned by the scene.
        if (!photonView.IsMine)
        {
            return;
        }

        // This code is only run by the master client.

        if (!gameStarted)
        {
            if (playerQueue.Count == 1 && !gameOver)
            {
                // Start the game
                gameStarted = true;
                BeginTurn();
    ...
    public void RegisterPlayer(PhotonView player)
    {
        playerQueue.Enqueue(player);
        Debug.Log("Player added to game queue, queue size: " + playerQueue.Count);
    }
...
public class TurnManager : MonoBehaviourPunCallbacks, IOnEventCallback
{
    ...
    public const byte EvMove = (byte) 1; 
    public const byte EvFinalMove = (byte) 2;
    public const byte EvRegisterPlayer = (byte) 3;
    ...
    {
        PhotonNetwork.RaiseEvent(EvRegisterPlayer, null, new RaiseEventOptions() { CachingOption = EventCaching.AddToRoomCache }, SendOptions.SendReliable);

        ProcessOnEvent(EvRegisterPlayer, null, PhotonNetwork.LocalPlayer.ActorNumber);
    }
